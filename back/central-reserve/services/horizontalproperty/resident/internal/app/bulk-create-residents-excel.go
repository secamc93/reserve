...
func (uc *residentUseCase) ImportResidentsFromExcel(ctx context.Context, businessID uint, filePath string) (*domain.ImportResidentsResult, error) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] app/bulk-create - Error abriendo Excel: %v\n", err)
	...
	if len(sheets) == 0 {
		fmt.Fprintf(os.Stderr, "[ERROR] app/bulk-create - El Excel no tiene hojas\n")
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] app/bulk-create - Error leyendo filas: %v\n", err)
	...
	if len(rows) < 2 {
		fmt.Fprintf(os.Stderr, "[ERROR] app/bulk-create - Excel sin datos: solo %d filas\n", len(rows))
	...
	for i, row := range rows {
		if i == 0 {
			...
			if len(row) < 3 {
				fmt.Fprintf(os.Stderr, "[ERROR] app/bulk-create - Faltan columnas en encabezado\n")
	...
	if len(result.Errors) > 0 {
		fmt.Fprintf(os.Stderr, "[WARN] app/bulk-create - %d errores de formato encontrados (se reportan en respuesta)\n", len(result.Errors))
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] app/bulk-create - Error obteniendo unidades: %v\n", err)
	...
	if len(missingUnits) > 0 {
		fmt.Fprintf(os.Stderr, "[WARN] app/bulk-create - %d unidades no existen en BD (se reportan en respuesta)\n", len(missingUnits))
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] app/bulk-create - Error precargando DNIs: %v\n", err)
	...
	if len(toCreate) > 0 {
		if err := uc.repo.CreateResidentsInBatch(ctx, toCreate); err != nil {
			fmt.Fprintf(os.Stderr, "[ERROR] app/bulk-create - Error creando residentes: %v\n", err)
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "[ERROR] app/bulk-create - Error releyendo IDs de DNIs creados: %v\n", err)
	...
	for _, entry := range blankEntries {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "[ERROR] app/bulk-create - Error creando residente sin DNI: %v\n", err)
	...
	if len(toPivot) > 0 {
		if err := uc.repo.CreateResidentUnitsInBatch(ctx, toPivot); err != nil {
			fmt.Fprintf(os.Stderr, "[ERROR] app/bulk-create - Error creando pivotes: %v\n", err)
