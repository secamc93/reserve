...
func (u *VotesUseCase) CreateVote(ctx context.Context, dto domain.CreateVoteDTO) (*domain.VoteDTO, error) {
	...
	// Publicar voto en cache para SSE en tiempo real
	if err := u.cache.PublishVote(dto.VotingID, *voteDTO); err != nil {
		u.logger.Error(ctx).Err(err).Uint("voting_id", dto.VotingID).Msg("Error publicando voto en cache")
		// No retornamos error, el voto ya fue creado exitosamente
	}
