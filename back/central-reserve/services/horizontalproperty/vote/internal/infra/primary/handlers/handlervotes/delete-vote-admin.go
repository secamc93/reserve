...
func (h *VotesHandler) DeleteVoteAdmin(c *gin.Context) {
	// Extraer vote_id del path
	voteIDParam := c.Param("vote_id")
	voteID, err := strconv.ParseUint(voteIDParam, 10, 32)
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/delete-vote-admin.go - Error parseando vote_id: %v\n", err)
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/delete-vote-admin.go - Error parseando voting_id: %v\n", err)
	...
	vote, err := h.votingUseCase.GetVoteByID(c.Request.Context(), uint(voteID))
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/delete-vote-admin.go - Error obteniendo voto: %v\n", err)
	...
	if vote.VotingID != uint(votingID) {
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/delete-vote-admin.go - Voto no pertenece a la votación: vote_voting_id=%d, expected_voting_id=%d\n", vote.VotingID, uint(votingID))
	...
	if err := h.votingUseCase.DeleteVote(c.Request.Context(), uint(voteID)); err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/delete-vote-admin.go - Error eliminando voto: %v\n", err)
	...
	h.logger.Info().
		...
		Uint("property_unit_id", vote.PropertyUnitID).
