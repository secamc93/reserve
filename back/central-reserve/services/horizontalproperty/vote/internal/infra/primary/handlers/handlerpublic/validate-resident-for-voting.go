...
func (h *PublicHandler) ValidateResidentForVoting(c *gin.Context) {

	// Obtener y validar token de votación pública
	authHeader := c.GetHeader("Authorization")
	if authHeader == "" {
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/validate-resident-for-voting.go - Token no proporcionado\n")
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/validate-resident-for-voting.go - Token inválido: %v\n", err)
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/validate-resident-for-voting.go - Error validando request: %v\n", err)
	...
	resident, err := h.publicUseCase.ValidateResidentForVoting(c.Request.Context(), hpID, req.PropertyUnitID, req.Dni)
	if err != nil {
		...
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/validate-resident-for-voting.go - Error validando residente: property_unit=%d, dni=%s, error=%v\n",
