...
//	@Router			/horizontal-properties/voting-groups/{group_id}/votings [post]
func (h *VotingsHandler) CreateVoting(c *gin.Context) {
	idParam := c.Param("group_id")
	id64, err := strconv.ParseUint(idParam, 10, 32)
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/create-voting.go - Error en handler: %v\n", err)
	...
	responseData := mappers.MapVotingDTOToResponse(created)
