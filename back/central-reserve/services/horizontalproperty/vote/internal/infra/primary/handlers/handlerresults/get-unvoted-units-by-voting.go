...
// @Security		BearerAuth
// @Router			/horizontal-properties/voting-groups/{group_id}/votings/{voting_id}/unvoted-units [get]
func (h *ResultsHandler) GetUnvotedUnitsByVoting(c *gin.Context) {
	// Obtener business_id del token
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/get-unvoted-units-by-voting.go - Error parseando group_id: %v\n", err)
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/get-unvoted-units-by-voting.go - Error parseando voting_id: %v\n", err)
		c.JSON(http.StatusBadRequest, response.ErrorResponse{
			Success: false,
	...
	voting, err := h.votingsUseCase.GetVotingByID(c.Request.Context(), uint(businessID), uint(groupID), uint(votingID))
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/get-unvoted-units-by-voting.go - Error obteniendo votación: %v\n", err)
	...
	unvotedUnits, err := h.resultsUseCase.GetUnvotedUnitsByVoting(c.Request.Context(), uint(votingID), unitNumberFilter)
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/get-unvoted-units-by-voting.go - Error obteniendo unidades sin votar: %v\n", err)
