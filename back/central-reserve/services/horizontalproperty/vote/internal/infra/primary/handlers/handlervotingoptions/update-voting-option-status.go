...
func (h *VotingOptionsHandler) UpdateVotingOptionStatus(c *gin.Context) {
	idParam := c.Param("option_id")
	id64, err := strconv.ParseUint(idParam, 10, 32)
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/update-voting-option-status.go - Error en handler: %v\n", err)
