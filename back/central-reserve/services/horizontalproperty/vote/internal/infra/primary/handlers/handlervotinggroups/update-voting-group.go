...
func (h *VotingGroupsHandler) UpdateVotingGroup(c *gin.Context) {
	idParam := c.Param("group_id")
	id64, err := strconv.ParseUint(idParam, 10, 32)
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/update-voting-group.go - Error en handler: %v\n", err)
	...
	responseData := mappers.MapVotingGroupDTOToResponse(updated)
