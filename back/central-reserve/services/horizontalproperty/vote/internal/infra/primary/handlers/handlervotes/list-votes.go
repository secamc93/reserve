...
func (h *VotesHandler) ListVotes(c *gin.Context) {
	idParam := c.Param("voting_id")
	id64, err := strconv.ParseUint(idParam, 10, 32)
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/list-votes.go - Error en handler: %v\n", err)
	...
	responseData := mappers.MapVoteDTOsToResponses(votes)
