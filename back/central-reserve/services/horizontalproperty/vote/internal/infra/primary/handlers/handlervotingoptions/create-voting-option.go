...
func (h *VotingOptionsHandler) CreateVotingOption(c *gin.Context) {
	idParam := c.Param("voting_id")
	id64, err := strconv.ParseUint(idParam, 10, 32)
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/create-voting-option.go - Error en handler: %v\n", err)
	...
	responseData := mappers.MapVotingOptionDTOToResponse(created)
