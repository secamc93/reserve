...
//	@Router			/horizontal-properties/voting-groups/{group_id}/votings/{voting_id}/deactivate [patch]
func (h *VotingsHandler) DeactivateVotingHandler(c *gin.Context) {
	idParam := c.Param("voting_id")
	id64, err := strconv.ParseUint(idParam, 10, 32)
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/deactivate-voting-handler.go - Error en handler: %v\n", err)
	...
	if err := h.votingUseCase.DeactivateVoting(c.Request.Context(), uint(id64)); err != nil {
		...
		h.logger.Error().
			Err(err).
			Uint("voting_id", uint(id64)).
			Msg("Error desactivando votación")
