...
func (h *VotingsHandler) UpdateVoting(c *gin.Context) {
	idParam := c.Param("voting_id")
	id64, err := strconv.ParseUint(idParam, 10, 32)
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/update-voting.go - Error en handler: %v\n", err)
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/update-voting.go - Error en handler: %v\n", err)
		h.logger.Error().Err(err).Uint("voting_id", uint(id64)).Str("title", req.Title).Msg("Error actualizando votación")
	...
	responseData := mappers.MapVotingDTOToResponse(updated)
