...
func (h *PublicHandler) GetPublicUnitsWithResidents(c *gin.Context) {

	// Obtener y validar token de autenticación de votación
	authHeader := c.GetHeader("Authorization")
	if authHeader == "" {
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/get-public-units-with-residents.go - Token no proporcionado\n")
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/get-public-units-with-residents.go - Token inválido: %v\n", err)
	...
	units, err := h.sharedUseCase.GetUnitsWithResidents(c.Request.Context(), hpID)
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlers/get-public-units-with-residents.go - Error listando unidades: hp_id=%d, error=%v\n", hpID, err)
