...
func (h *VotingHandler) ListVotings(c *gin.Context) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlervote/list-votings.go - Error en handler: %v\n", err)
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlervote/list-votings.go - Error en handler: %v\n", err)
		h.logger.Error().Err(err).Uint("group_id", uint(id64)).Msg("Error listando votaciones")
