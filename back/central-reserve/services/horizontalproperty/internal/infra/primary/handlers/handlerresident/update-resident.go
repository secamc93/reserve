...
func (h *ResidentHandler) UpdateResident(c *gin.Context) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlerresident/update-resident.go - Error en handler: %v\n", err)
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlerresident/update-resident.go - Error en handler: %v\n", err)
		h.logger.Error().Err(err).Uint("resident_id", uint(resID)).Msg("Error validando datos del request")
	...
	if err != nil {
		...
		fmt.Fprintf(os.Stderr, "[ERROR] handlerresident/update-resident.go - Error en handler: %v\n", err)
		h.logger.Error().Err(err).Uint("resident_id", uint(resID)).Interface("dto", dto).Msg("Error actualizando residente")
