...
func (h *ResidentHandler) DeleteResident(c *gin.Context) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ERROR] handlerresident/delete-resident.go - Error en handler: %v\n", err)
	...
	if err != nil {
		...
		fmt.Fprintf(os.Stderr, "[ERROR] handlerresident/delete-resident.go - Error en handler: %v\n", err)
		h.logger.Error().Err(err).Uint("resident_id", uint(resID)).Msg("Error eliminando residente")
