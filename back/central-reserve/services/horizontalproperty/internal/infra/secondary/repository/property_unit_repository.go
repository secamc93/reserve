...
func (r *Repository) CreatePropertyUnit(ctx context.Context, unit *domain.PropertyUnit) (*domain.PropertyUnit, error) {
	model := &models.PropertyUnit{
		BusinessID: unit.BusinessID, Number: unit.Number, Floor: unit.Floor, Block: unit.Block,
		UnitType: unit.UnitType, Area: unit.Area, Bedrooms: unit.Bedrooms, Bathrooms: unit.Bathrooms,
		Description: unit.Description, IsActive: unit.IsActive,
	}
...
func (r *Repository) ListPropertyUnits(ctx context.Context, filters domain.PropertyUnitFiltersDTO) (*domain.PaginatedPropertyUnitsDTO, error) {
	...
	for i, m := range ms {
		units[i] = domain.PropertyUnitListDTO{ID: m.ID, Number: m.Number, Floor: m.Floor, Block: m.Block,
			UnitType: m.UnitType, Area: m.Area, Bedrooms: m.Bedrooms, Bathrooms: m.Bathrooms, IsActive: m.IsActive}
	}
	return &domain.PaginatedPropertyUnitsDTO{Units: units, Total: total, Page: filters.Page, PageSize: filters.PageSize,
		TotalPages: int(math.Ceil(float64(total) / float64(filters.PageSize)))}, nil
}

func (r *Repository) UpdatePropertyUnit(ctx context.Context, id uint, unit *domain.PropertyUnit) (*domain.PropertyUnit, error) {
	model := &models.PropertyUnit{Number: unit.Number, Floor: unit.Floor, Block: unit.Block, UnitType: unit.UnitType,
		Area: unit.Area, Bedrooms: unit.Bedrooms, Bathrooms: unit.Bathrooms, Description: unit.Description, IsActive: unit.IsActive}
...
func mapPropertyUnitToDomain(m *models.PropertyUnit) *domain.PropertyUnit {
	return &domain.PropertyUnit{ID: m.ID, BusinessID: m.BusinessID, Number: m.Number, Floor: m.Floor, Block: m.Block,
		UnitType: m.UnitType, Area: m.Area, Bedrooms: m.Bedrooms, Bathrooms: m.Bathrooms, Description: m.Description,
