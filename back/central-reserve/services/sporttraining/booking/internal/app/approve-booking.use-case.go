...
func (uc *bookingUseCase) ApproveBooking(ctx context.Context, dto domain.ApproveBookingDTO) (*domain.Booking, error) {
	ctx = log.WithFunctionCtx(ctx, "ApproveBooking")
	...
	// Obtener estado "approved"
	approvedStatus, err := uc.repo.GetBookingStatusByCode(ctx, "approved")
	if err != nil {
		uc.logger.Error(ctx).Err(err).Msg("Error obteniendo estado approved")
		return nil, err
	}

	// Actualizar reserva
	now := time.Now()
	booking.BookingStatusID = approvedStatus.ID
	...
	if err != nil {
		uc.logger.Error(ctx).Err(err).Msg("Error aprobando reserva")
		return nil, err
	}

	uc.logger.Info(ctx).Msgf("Reserva %d aprobada por coach %d", dto.ID, dto.CoachID)
	return updated, nil
}
