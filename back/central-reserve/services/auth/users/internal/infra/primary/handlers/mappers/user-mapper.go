...
func ToCreateUserDTO(req request.CreateUserRequest) (domain.CreateUserDTO, error) {
	...
	// Convertir archivo multipart a FileUpload
	avatarFile, err := MultipartToFileUpload(req.AvatarFile)
	if err != nil {
		return domain.CreateUserDTO{}, err
	}
...
func ToUpdateUserDTO(req request.UpdateUserRequest) (domain.UpdateUserDTO, error) {
	businessIDs := make([]uint, 0)
	if len(req.BusinessIDs) > 0 {
		businessIDs = append(businessIDs, req.BusinessIDs...)
	} else if req.BusinessIDsRaw != "" {
	...
	avatarFile, err := MultipartToFileUpload(req.AvatarFile)
	if err != nil {
		return domain.UpdateUserDTO{}, err
	}
