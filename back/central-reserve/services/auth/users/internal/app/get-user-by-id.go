...
func (uc *UserUseCase) GetUserByID(ctx context.Context, id uint) (*userdomain.UserDTO, error) {
	...
	if err != nil {
		...
		userDTO.Roles = make([]userdomain.RoleDTO, len(roles))
		for i, role := range roles {
			userDTO.Roles[i] = userdomain.RoleDTO{
				ID:          role.ID,
				Name:        role.Name,
	...
	if err != nil {
		...
		userDTO.Businesses = make([]userdomain.BusinessDTO, len(businesses))
		for i, business := range businesses {
			...
			userDTO.Businesses[i] = userdomain.BusinessDTO{
				ID:                 business.ID,
				Name:               business.Name,
				Code:               business.Code,
	...
	if len(userDTO.Businesses) > 0 {
		assignments := make([]userdomain.BusinessRoleAssignmentDetailed, 0, len(userDTO.Businesses))
		for _, b := range userDTO.Businesses {
			assignment := userdomain.BusinessRoleAssignmentDetailed{
				BusinessID:   b.ID,
				BusinessName: b.Name,
			}
			if b.Role != nil {
	...
	if isSuperUser {
		...
		userDTO.BusinessRoleAssignments = append([]userdomain.BusinessRoleAssignmentDetailed{superUserAssignment}, userDTO.BusinessRoleAssignments...)
	}
