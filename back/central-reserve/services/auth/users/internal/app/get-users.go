...
import (
	userdomain "central_reserve/services/auth/users/internal/domain"
	"context"
	"fmt"
	"strings"
)
...
func (uc *UserUseCase) GetUsers(ctx context.Context, filters userdomain.UserFilters) (*userdomain.UserListDTO, error) {
	uc.log.Info().
	...
	userDTOs := make([]userdomain.UserDTO, len(users))
	for i, user := range users {
		...
		if err != nil {
			...
			userDTOs[i].Roles = make([]userdomain.RoleDTO, len(roles))
			for j, role := range roles {
				userDTOs[i].Roles[j] = userdomain.RoleDTO{
					ID:          role.ID,
					Name:        role.Name,
		...
		if err != nil {
			...
			assignments := make([]userdomain.BusinessRoleAssignmentDetailed, 0)
			businessMap := make(map[uint]userdomain.BusinessDTO)
			...
			if err == nil {
				for _, business := range businesses {
					...
					businessMap[business.ID] = userdomain.BusinessDTO{
						ID:                 business.ID,
						Name:               business.Name,
						Code:               business.Code,
			...
			businessDTOs := make([]userdomain.BusinessDTO, 0, len(businessMap))
		...
		if isSuperUser {
			...
			superUserAssignment := userdomain.BusinessRoleAssignmentDetailed{
				BusinessID:   0,
			...
			userDTOs[i].BusinessRoleAssignments = append([]userdomain.BusinessRoleAssignmentDetailed{superUserAssignment}, userDTOs[i].BusinessRoleAssignments...)
		}
	}
	...
	userListDTO := &userdomain.UserListDTO{
		Users:      userDTOs,
		Total:      total,
		Page:       filters.Page,
