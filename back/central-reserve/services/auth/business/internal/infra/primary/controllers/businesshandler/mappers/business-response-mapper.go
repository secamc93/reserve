...
func RequestToDTO(req request.BusinessRequest) (domain.BusinessRequest, error) {
	logoFile, err := MultipartToFileUpload(req.LogoFile)
	if err != nil {
		return domain.BusinessRequest{}, err
	}

	navbarImageFile, err := MultipartToFileUpload(req.NavbarImageFile)
	if err != nil {
		return domain.BusinessRequest{}, err
	}
...
func RequestToUpdateDTO(req request.BusinessRequest) (domain.UpdateBusinessRequest, error) {
	logoFile, err := MultipartToFileUpload(req.LogoFile)
	if err != nil {
		return domain.UpdateBusinessRequest{}, err
	}
...
func UpdateRequestToUpdateDTO(req request.UpdateBusinessRequest) (domain.UpdateBusinessRequest, error) {
	logoFile, err := MultipartToFileUpload(req.LogoFile)
	if err != nil {
		return domain.UpdateBusinessRequest{}, err
	}
