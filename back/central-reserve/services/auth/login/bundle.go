...
func New(database db.IDatabase, env env.IConfig, logger log.ILogger, jwtService domain.IJWTService, v1Group *gin.RouterGroup) {
	repo := repository.New(database, logger)
	useCase := app.New(repo, jwtService, logger, env)
