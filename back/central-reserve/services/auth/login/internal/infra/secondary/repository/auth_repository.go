...
import (
	...
	"central_reserve/services/auth/login/internal/infra/secondary/repository/mappers"
	"context"
...
// GetRoleByID obtiene un rol por su ID y lo mapea a domain.Role (versión usada en login)
func (r *Repository) GetRoleByID(ctx context.Context, roleID uint) (*domain.Role, error) {
	...
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			r.logger.Warn().Uint("role_id", roleID).Msg("Rol no encontrado")
			return nil, nil
		}
...
// GetBusinessByID obtiene un business por su ID y lo mapea a domain.BusinessInfo
func (r *Repository) GetBusinessByID(ctx context.Context, businessID uint) (*domain.BusinessInfo, error) {
	...
	businessInfo := &domain.BusinessInfo{
		ID:                 business.ID,
		Name:               business.Name,
		Code:               business.Code,
		BusinessTypeID:     business.BusinessTypeID,
		BusinessType:       domain.BusinessTypeInfo{ID: business.BusinessType.ID, Name: business.BusinessType.Name, Code: business.BusinessType.Code},
	...
	// Completar BusinessTypeName / Code si están disponibles
	if business.BusinessType.ID != 0 {
		businessInfo.BusinessType.Name = business.BusinessType.Name
		businessInfo.BusinessType.Code = business.BusinessType.Code
	}
