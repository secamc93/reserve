...
export class AuthRepositoryHttp implements AuthRepository {
  ...
  private mapBusinessFromBackend(b: any): Business {
    return {
      ...
      tertiaryColor: b.tertiary_color,
      quaternaryColor: b.quaternary_color,
      navbarImageURL: b.navbar_image_url,
  ...
  async login(credentials: LoginCredentials): Promise<LoginResponse> {
    try {
      console.log('🔐 AuthRepositoryHttp: Iniciando login');
      console.log('🔐 AuthRepositoryHttp: Email:', credentials.email);

      const response = await this.httpClient.post('/api/v1/auth/login', credentials);

      console.log('🔐 AuthRepositoryHttp: Respuesta del servidor:', response);
      ...
      if (response.success && response.data) {
        ...
        console.log('🔐 AuthRepositoryHttp: Login exitoso, token y usuario guardados');
        ...
        console.error('🔐 AuthRepositoryHttp: Respuesta inválida del servidor');
        throw new Error('Respuesta inválida del servidor');
      }
    } catch (error) {
      console.error('🔐 AuthRepositoryHttp: Error en login:', error);
      throw error;
    }
  }
  ...
  async getUserRolesPermissions(): Promise<ApiRolesPermissionsResponse> {
    try {
      console.log('🔐 AuthRepositoryHttp: Obteniendo roles y permisos');

      const response = await this.httpClient.get('/api/v1/auth/roles-permissions');

      console.log('🔐 AuthRepositoryHttp: Respuesta de roles y permisos:', response);

      if (response.success && response.data) {
        return response as ApiRolesPermissionsResponse;
      } else {
        console.error('🔐 AuthRepositoryHttp: Respuesta inválida de roles y permisos');
        throw new Error('Respuesta inválida de roles y permisos');
      }
    } catch (error) {
      console.error('🔐 AuthRepositoryHttp: Error obteniendo roles y permisos:', error);
      throw error;
    }
  }
  ...
  async changePassword(passwordData: ChangePasswordData): Promise<any> {
    try {
      console.log('AuthRepositoryHttp: Cambiando contraseña');
      const response = await this.httpClient.post('/api/v1/auth/change-password', passwordData);
      console.log('AuthRepositoryHttp: Contraseña cambiada:', response);
      return response;
    } catch (error) {
      console.error('AuthRepositoryHttp: Error cambiando contraseña:', error);
      throw error;
    }
  }

  logout(): void {
    console.log('🔐 AuthRepositoryHttp: Cerrando sesión');
    if (typeof window !== 'undefined') {
