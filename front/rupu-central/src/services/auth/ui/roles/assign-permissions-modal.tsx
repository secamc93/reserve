...
import { getPermissionsListAction, getRolePermissionsAction, removeRolePermissionAction } from '@/services/auth/infrastructure/actions';
        ...
        {fetchLoading ? (
          ...
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {permissions.map(permission => (
              <label
                key={permission.id}
                className={`flex items-start gap-3 p-3 border rounded-lg cursor-pointer hover:bg-gray-50 ${selectedPermissions.includes(permission.id) ? 'bg-blue-50 border-blue-500' : 'border-gray-200'
                  }`}
              >
                <input
                  type="checkbox"
