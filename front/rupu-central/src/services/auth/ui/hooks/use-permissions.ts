...
import { UserPermissions, RESOURCE_ROUTE_MAP } from '../../domain/entities/permissions.entity';

/**
 * Hook para gestionar permisos del usuario basados en recursos
 * Consulta el endpoint /auth/roles-permissions con el business_token
 */
export function usePermissions() {
  ...
  const loadPermissions = useCallback(async () => {
    ...
    try {
      ...
      const { getPermissionsAction } = await import('../../infrastructure/actions/permissions/get-permissions.action');
  ...
  const hasResource = useCallback((resourceName: string): boolean => {
    ...
    if (permissions.isSuperAdmin) return true; // Super admin tiene acceso a todo
  ...
  const getResourceActions = useCallback((resourceName: string): string[] => {
    ...
    if (permissions.isSuperAdmin) return ['Create', 'Read', 'Update', 'Delete']; // Super admin tiene todas las acciones
  ...
  const getActiveResources = useCallback((): string[] => {
    if (!permissions) return [];
    if (permissions.isSuperAdmin) return Object.keys(RESOURCE_ROUTE_MAP); // Super admin tiene todos los recursos
